...
import 'package:fast_app_base/screen/main/tab/todo/vo_write_to_result.dart';
...
class _WriteTodoDialogState extends DialogState<WriteTodoDialog> with AfterLayoutMixin {
  ...
  final textController = TextEditingController();   //입력값 컨트롤러
  final node = FocusNode();           //화면 키보드 노출여부

  @override
  void initState(){
  //일정 수정값이 들어오면 수정할수 있도록 데이터 작업
    if(widget.todoForEdit != null){
      _selectedDate = widget.todoForEdit!.dueDate;  //무조건 null이 아님으로 !설정
  ...
  Widget build(BuildContext context) {
    return BottomDialogScaffold(body: RoundedContainer(
      child: Column(
        children: [
          // emptyExpanded == Spacer() 같은 역활 위젯이다.
          Row(children: ['할일을 작성해주세요.'.text.size(18).bold.make(),
            spacer,
            _selectedDate.formattedDate2.text.make(),
  ...
  void _selectDate() async{
    final date = await showDatePicker(
      ...
      lastDate : DateTime.now().add(const Duration(days:365*3)),   //3년까지만
    );
    if(date!=null){
      setState(() {
        _selectedDate = date;
